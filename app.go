...
func (a *App) AddSeating(code, description string, debit, credit float64, detail, date, numDoc, asn string, categoryID int) error {
    _, err := a.db.Exec(`
        INSERT INTO seating (code, description, debit, credit, detail, date, num_doc, asn, id_category)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, code, description, debit, credit, detail, date, numDoc, asn, categoryID)
    return err
}
...
func (a *App) ListSeatings() ([]Seating, error) {
    rows, err := a.db.Query(`
        SELECT s.id, s.code, s.description, s.debit, s.credit, s.detail, s.date, s.num_doc, s.asn, s.id_category, c.name
        FROM seating s
    ...
    for rows.Next() {
        var seating Seating
        err := rows.Scan(&seating.ID, &seating.Code, &seating.Description, &seating.Debit, &seating.Credit, &seating.Detail, &seating.Date, &seating.NumDoc, &seating.ASN, &seating.CategoryID, &seating.Category)
        if err != nil {
            return nil, err
        }
...
func (a *App) EditSeating(ID int, code, description string, debit, credit float64, detail, date, numDoc, asn string, categoryID int) error {
    _, err := a.db.Exec(`
        UPDATE seating SET code=?, description=?, debit=?, credit=?, detail=?, date=?, num_doc=?, asn=?, id_category=?
        WHERE id=?`, code, description, debit, credit, detail, date, numDoc, asn, categoryID, ID)
    return err
}